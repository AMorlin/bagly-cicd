...
import { BRAZILIAN_AIRPORTS, getAirportByCode } from './airports'
...
describe('BRAZILIAN_AIRPORTS', () => {
  ...
  it('should include major Brazilian airports', () => {
    ...
    expect(codes).toContain('SSA') // Salvador
    expect(codes).toContain('FOR') // Fortaleza
  })

  it('should have IATA 3-letter codes', () => {
    for (const airport of BRAZILIAN_AIRPORTS) {
      expect(airport.code).toMatch(/^[A-Z]{3}$/)
    }
  })

  it('should have São Paulo airports', () => {
    const spAirports = BRAZILIAN_AIRPORTS.filter((a) => a.city === 'São Paulo')
    expect(spAirports.length).toBeGreaterThanOrEqual(2)
    const codes = spAirports.map((a) => a.code)
    expect(codes).toContain('GRU')
    expect(codes).toContain('CGH')
  })

  it('should have Rio de Janeiro airports', () => {
    const rjAirports = BRAZILIAN_AIRPORTS.filter((a) => a.city === 'Rio de Janeiro')
    expect(rjAirports.length).toBeGreaterThanOrEqual(2)
    const codes = rjAirports.map((a) => a.code)
...
describe('getAirportByCode', () => {
  it('should return airport for valid code', () => {
    ...
    expect(airport?.name).toContain('Guarulhos')
  })
  ...
  it('should find GIG airport', () => {
    const airport = getAirportByCode('GIG')
    expect(airport).toBeDefined()
    expect(airport?.city).toBe('Rio de Janeiro')
    expect(airport?.name).toContain('Galeão')
  })

  it('should find BSB airport', () => {
    const airport = getAirportByCode('BSB')
    expect(airport).toBeDefined()
    expect(airport?.city).toBe('Brasília')
  })
  ...
  it('should find all airports by their codes', () => {
    for (const airport of BRAZILIAN_AIRPORTS) {
      ...
      expect(found?.name).toBe(airport.name)
      expect(found?.city).toBe(airport.city)
    }
  })
})
